...
mem_init(void)
{
	...
	// but we just set up the mapping anyway. Permissions: kernel
	// RW, user NONE Your code goes here:
	boot_map_region(kern_pgdir,
	                KERNBASE,
	                -KERNBASE,
	                0,
	                PTE_W);  // -KERNBASE is the complement of KERNBASE (in
	                         // binary), so -KERNBASE == 2**32 - KERNBASE
...
page_init(void)
{
	...
	*/
	...
	for (size_t i = 0; i < npages; i++) {
		...
		// If current is page 0 or current is in a range already used
		if ((current == 0) ||
		    (current >= IOPHYSMEM && current < PADDR(boot_alloc(0)))) {
			continue;
		}

		// We need to divide because we are comparing the number of the page
...
page_alloc(int alloc_flags)
{
	...
	if (alloc_flags && ALLOC_ZERO) {
		// Create a new page with size PGSIZE & initialize every position with 0
...
page_free(struct PageInfo *pp)
{
	...
	pp->pp_link =
	        page_free_list;  // Add new page to the beginning of the list
	...
	if (!(pgdir[pg_dir_index] && PTE_P)) {  // If the directory does not exist
		if (create) {
			...
			pgdir[pg_dir_index] =
			        ...
			        PTE_W;  // Physical address Assignment with needed flags
		} else
			return NULL;
	}
...
{
	while (size > 0) {
		bool condition = (va % PTSIZE) == 0 && (pa % PTSIZE) == 0 &&
		                 size >= PTSIZE;
		int aux = condition ? PTSIZE : PGSIZE;
		...
		*v_addr = pa | perm | PTE_P;
		...
		if (condition) {
			// Large pages receive a special treatment
			pte_t *v_addr = pgdir + PDX(va);

			// The next line repeates code but in this part we value
			// readability rather than having no repetition.
			*v_addr = pa | perm | PTE_P | PTE_PS;
		} else {
			// Normal pages
			pte_t *v_addr = pgdir_walk(pgdir, (void *) va, true);
			if (v_addr == NULL) {
...
page_lookup(pde_t *pgdir, void *va, pte_t **pte_store)
{
	...
	if (v_addr == NULL ||
	    !(*v_addr && PTE_P)) {  // If fails or we do not have permissions
		return NULL;
	}
	...
	return pa2page(PTE_ADDR(
	        *v_addr));  // We want to return the physical address. ACA NO SE SI VA EL PTE_ADDR
}
...
page_remove(pde_t *pgdir, void *va)
{
	...
	if (!(*buf && PTE_P) ||
	    !page) {  // Page does not exist or we do not have permissions
		return;
	}
	...
	while (begin < end) {
		...
		if (page == NULL || (*page & PTE_P) == 0 ||
			...
			user_mem_check_addr =
			        ((begin < (uintptr_t) va)
			                 ? (uintptr_t) va
			                 : begin);  // set the 'user_mem_check_addr' variable to the first
